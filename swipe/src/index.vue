  ...
  methods: {
    ...
    translate (element, offset, speed, callback) {
      if (speed) {
        ...
        setTimeout(transitionEndCallback, speed + 100) // webkitTransitionEndmaybe not fire on lower version android.
      } else {
        element.style.webkitTransition = ''
        element.style.webkitTransform = `translate3d(${offset}px, 0, 0)`
      }
    ...
    reInitPages () {
      ...
      children.forEach(function (child, index) {
        ...
        _this.removeClass(child.$el, 'is-active')

        if (index === defaultIndex) {
          _this.addClass(child.$el, 'is-active')
        }
      })
